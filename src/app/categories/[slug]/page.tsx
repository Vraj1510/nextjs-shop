...
import { getCategory, getCategoryGames, getGame, getRecentGames } from '@/libs/apis';
...
const GameCategory = async (props: { params: { slug: string } }) => {
  ...
  const category=await getGame(slug);
    ...
    <>
      ...
      <section className={classNames.section}>
        ...
        <p className={classNames.subHeading}>
          Check our latest collection of <span className='text-primary'>{slug}</span> games
        </p>
...
const classNames = {
  ...
  title: 'text-2xl md:text-4xl lg:text-5xl font-extrabold tracking-tight text-white',
  subtitle: 'mt-2 md:mt-4 max-w-3xl text-sm md:text-xl text-gray-300',
  author: 'mt-4 md:mt-6 flex items-center',
  authorAvatar: 'flex-shrink-0 object-cover h-8 md:h-10 w-8 md:w-10 rounded-full',
